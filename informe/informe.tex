...
\begin{description}
  ...
  $\sim$3 personas/min por metro de puerta y el input dejaría de ser alcanzable.)
\end{description}
...
La distribución de tiempos de evacuación (ec.~\ref{eq:tevac}) para las siete capacidades se
...
\caption{Distribución de tiempos de evacuación para las capacidades del barrido
($N=40$ a $500$).}
\label{fig:evac-hist}
\end{figure}
...
200 & $199.0$ & $63.2 \pm 1.6$ & $129.3 \pm 8.5$ \\
300 & $298.8$ & $88.7 \pm 20.0$ & $243.5 \pm 102.8$ \\
400 & $399.0$ & $94.5 \pm 1.5$ & $251.4 \pm 3.6$ \\
500 & $499.0$ & $108.7 \pm 3.2$ & $305.2 \pm 17.9$ \\
\bottomrule
\end{tabular}
...
El edificio se vacía por completo en todo el rango explorado: los evacuados son $\ge N-1$
hasta $N=500$. El tiempo de evacuación crece con $N$ en dos regímenes. Hasta $N\approx 200$
el crecimiento es suave (multiplicar la capacidad por 5 apenas ronda el $\times$1.6 en el
promedio: las escaleras absorben el flujo). Entre $N=200$ y $N=300$ el \textbf{máximo} salta de
$129.3\pm8.5$ a $243.5\pm102.8$~\si{\second}: las escaleras pasan a trabajar \emph{saturadas},
con cola sostenida en sus bocas, y el tiempo del último evacuado queda gobernado por el drenaje
de esa cola (que crece con $N$) y no por la longitud del recorrido. La dispersión entre
realizaciones es máxima justo en ese cruce: en $N=300$, cuatro de las cinco realizaciones
terminan con máximos de 190--206~\si{\second}, pero en una se forma un atasco transitorio en la
boca de una escalera que se disuelve solo y estira el máximo a $427$~\si{\second} --- cerca del
régimen crítico, la realización manda. Pasado el cruce ($N=400$, $500$) la cola saturada domina
y la dispersión vuelve a bajar. La distribución es \textbf{bimodal}: un primer lóbulo corto
corresponde a los agentes de la planta baja y un segundo lóbulo lento a los de la planta alta,
que deben bajar la escalera \emph{switchback} (más larga y a velocidad reducida que un tramo
recto); el segundo lóbulo se desplaza hacia tiempos mayores y se ensancha al crecer $N$, hasta
dominar la distribución en las capacidades grandes.
...
\begin{itemize}
  ...
  saturadas ($N\gtrsim 300$), donde el máximo se despega del promedio. En el cruce entre
  regímenes la sensibilidad a la realización es máxima (en $N=300$ una de cinco corridas formó
  un atasco transitorio que duplicó el máximo). La distribución es bimodal: los agentes de la
  planta baja salen rápido y los de la planta alta forman el lóbulo lento, que crece y se
  ensancha con $N$ hasta dominar.
