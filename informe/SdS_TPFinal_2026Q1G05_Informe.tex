...
Sobre esta ampliación se estudia el escenario \emph{Escuela}, en el que cada sub-escenario varía
un \emph{input} y mide un \emph{observable}. Definimos a continuación los tres observables que se
usan en los resultados.
...
distribuyen uniformemente a lo largo de $T_a \in \{1,5,10\}$~\si{\minute}, con $N_{\max}$ fijo. El
...
\addlinespace
Geometría & Cada pared, salida, aula, generador y servidor lleva su planta $z$. Nuevo tipo
...
Cada elemento de geometría (pared, salida, aula, generador y servidor) lleva una única coordenada
...
La detección de vecinos es independiente por planta salvo sobre las escaleras. Una \emph{fachada}
...
donde la $z$ se perdía en silencio, con un \emph{valor por defecto} $z=0$, y la simulación resultante era
...
es visible''; al llegar al pie el salto (de ruteo) avanza ahora explícitamente al tope. La interpolación de la
$z$ exigía estar estrictamente entre plantas y nunca arrancaba al pisar el pie; se la detecta
también cuando el \emph{foot-target} apunta a otra planta. Y tanto los servidores como los generadores
...
estado, porque los \emph{valores por defecto} silenciosos producen simulaciones que corren y ``se ven bien''
...
aplica un \emph{sesgo} lateral que separa los carriles de subida y bajada (contraflujo). En la
...
ruteaba al pie a ras del piso por dentro de la propia huella--- y el cambio de salto del pie al tope ocurría
...
sin cruzarla, durante decenas de segundos. El diagnóstico descartó el ruteo: el salto era estable y
...
\begin{itemize}
  ...
  con un kiosco (servidor con cola) y una salida propia.
...
Las aulas se modelan como servidores de tipo \texttt{CLASSROOM} (recinto colectivo con una sesión
de clase). Cada escalera tiene un ancho holgado y un factor de velocidad reducida
(Tabla~\ref{tab:params}); sobre los tramos la velocidad efectiva medida es
...
emergencia mayor que la de la caminata normal, del orden de las velocidades deseadas usadas en
modelos de escape \cite{helbing2000}. Los parámetros de la simulación se resumen en la
Tabla~\ref{tab:params}. El paso de integración efectivo es
$\Delta t = \min\!\bigl(\Delta t_{\text{esc}},\, r_{\min}/(2\max(v_d,v_e))\bigr)$: el modelo
acota el $\Delta t$ del escenario a un valor estable para el perfil más rápido presente, que
resulta por lo tanto menor en la evacuación que en la caminata normal. La salida se registra cada
$\Delta t_{\text{out}}$.
...
\begin{description}
  ...
  (velocidad deseada de emergencia, Tabla~\ref{tab:params}). El input es la capacidad total $N$, con barrido
  $N\in\{40,80,120,200,300,400,500\}$ (hasta $\sim$31 alumnos por aula, $\approx$0.5
  agentes/\si{\metre\squared} de ocupación inicial). El tiempo total de simulación es de
  \SI{400}{\second} para $N\le 120$ y escala linealmente con $N$ por encima
  ($400 + 1.2\,(N-120)$~\si{\second}), de modo que toda capacidad tenga margen para vaciarse.
  ...
  $T_a\in\{1,5,10\}$~\si{\minute} a $N_{\max}$ fijo, equivalente a un caudal
  $Q=N_{\max}/T_a\in\{120,24,12\}$ agentes por minuto (ec.~\ref{eq:caudal}). El tiempo total es
  $T_a + \SI{250}{\second}$.
  \item[Estudio complementario.] Sobre el mismo sub-escenario de Ingreso se varía $N_{\max}\in
  \{60,120,180,240,300\}$ a ventana fija $T_a=\SI{5}{\minute}$, para ver cómo escala la
  congestión con la cantidad de gente. (A esta ventana las puertas entregan el caudal pedido
  completo; con ventanas más cortas el generador se topa con su tope configurado de
  $\sim$3 personas por minuto por metro de puerta y el input dejaría de ser alcanzable.)
\end{description}
...
La distribución de tiempos de evacuación (ec.~\ref{eq:tevac}) para las siete capacidades del
barrido se muestra en la Figura~\ref{fig:evac-curves-band} como curvas de densidad con banda
$\pm\sigma$ entre las 5 realizaciones (por legibilidad, sólo $N=80,200,300,400,500$); el escalar
(promedio y máximo) en función de $N$ se muestra en la Figura~\ref{fig:evac-scalar}.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{evac_curves_band.png}
\caption{Distribución de tiempos de evacuación como curvas de densidad superpuestas, con banda
$\pm\sigma$ entre realizaciones, para $N=80,200,300,400,500$.}
\label{fig:evac-curves-band}
\end{figure}
...
El edificio se vacía por completo en todo el rango explorado ($N$ hasta $500$). La distribución
es \textbf{bimodal}: un primer lóbulo, corto y angosto, agrupa a los agentes que arrancan en
planta baja y salen directo por una puerta, sin pasar por ninguna escalera; un segundo lóbulo,
más lento y ancho, corresponde a los agentes de planta alta, que deben bajar por la escalera
\emph{switchback} (tramo más largo y a velocidad reducida que uno recto). A medida que $N$
crece, el segundo lóbulo gana peso relativo y se desplaza hacia tiempos mayores, hasta dominar
la distribución en las capacidades grandes.

Ese desplazamiento es lo que explica el escalar (Figura~\ref{fig:evac-scalar}): el promedio
crece de forma suave con $N$ (de $38.9$ a $108.7$~\si{\second}, $\times2.8$ entre $N=40$ y
$N=500$), pero el \textbf{máximo crece mucho más rápido} (de $57.0$ a $305.2$~\si{\second},
$\times5.4$) porque queda gobernado por el agente más rezagado del lóbulo lento --- el que
arranca más lejos de la escalera o encuentra más cola en su boca. La brecha entre ambos crece
con $N$ y, a partir de $N\approx200$, lo hace a un ritmo aproximadamente constante de
$\sim$40--45~\si{\second} cada 100 agentes adicionales: reflejo de que, a esas capacidades, la
escalera empieza a trabajar saturada, sostiene una cola en su boca y el tiempo del último
evacuado queda gobernado por el drenaje de esa cola y no por la longitud del recorrido.

De los $N$ agentes no evacúan todos: queda sin evacuar $\approx 1$ por corrida, y ese remanente
\textbf{no crece con $N$} ($1.0$ a $1.2$ en todo el barrido, incluso con $N=500$). El descenso
por la escalera \emph{switchback}
...
en que termina la corrida (la salida deja de escribir cuadros al vaciarse el edificio): un caso
borde del criterio operativo de evacuado (ec.~\ref{eq:tevac}), no una evacuación fallida. De
manera esporádica (en corridas aisladas de las 35 del barrido) reaparece en la planta baja el
...
\begin{tabular}{@{}rrll@{}}
\toprule
$T_a$ [\si{\minute}] & $Q$ [ag.\,\si{\per\minute}] & Ocupación pico & Ocupación promedio \\
\midrule
...
\caption{Población en la zona vs. tiempo para $N_{\max}=60$ a $300$ (ventana de
5~minutos).}
\label{fig:nmax-pob}
\end{figure}
...
240 & $78.8 \pm 2.3$ & $51.4 \pm 1.0$ \\
300 & $104.0 \pm 3.7$ & $70.8 \pm 3.3$ \\
\bottomrule
\end{tabular}
...
La ocupación muestra dos regímenes. Hasta $N_{\max}\approx 180$ el crecimiento es más que
proporcional: triplicar la población ($60\to180$) multiplica el pico por $\sim$11 ($4.6\to51.4$)
--- el servidor del kiosco deja de dar abasto y empieza a formarse cola. A partir de ahí el
kiosco trabaja saturado (atiende a tasa fija) y cada agente adicional simplemente se suma a la
cola: el pico crece de forma $\sim$lineal con pendiente $\approx 0.44$ agentes de pico por
agente de $N_{\max}$ ($51.4 \to 78.8 \to 104.0$ para $180 \to 240 \to 300$). En el extremo
$N_{\max}=300$ el pico de $104.0\pm3.7$ personas en $\SI{120}{\metre\squared}$ ya bordea
1~persona/\si{\metre\squared} promediada sobre toda la zona, con densidades locales mucho
mayores en el frente de la cola (Figura~\ref{fig:dens-ing}).
...
\begin{itemize}
  ...
  apilada.
  \item La escalera \emph{switchback} con descanso confina a los agentes dentro de su huella y
  separa los carriles de subida y bajada; la coordenada $z$ evoluciona de forma monótona a lo
  largo del descenso.
  \item En la evacuación, el edificio se vacía completo en todo el rango explorado
  ($N$ hasta $500$, $\approx$0.5 agentes/\si{\metre\squared} inicial) y el tiempo de evacuación
  crece con la capacidad en \textbf{dos regímenes}: suave mientras las escaleras absorben el
  flujo ($N\lesssim 200$) y gobernado por el drenaje de la cola en sus bocas cuando trabajan
  saturadas ($N\gtrsim 300$), donde el máximo se despega del promedio. La distribución es
  bimodal: los agentes de la planta baja salen rápido y los de la planta alta forman el
  lóbulo lento, que crece y se ensancha con $N$ hasta dominar.
  \item En el ingreso, la ocupación de la zona observada crece al acortar la ventana de llegada
  (mayor caudal). En el estudio complementario crece más que proporcionalmente con $N_{\max}$
  hasta saturar el kiosco ($N_{\max}\approx 180$) y luego $\sim$linealmente: el servidor
  saturado atiende a tasa fija y el excedente se acumula en la cola.
  \item En promedio $\approx 1$ agente por corrida no evacúa, y ese remanente \textbf{no crece
  con $N$}: casi siempre es el último agente todavía en tránsito hacia la salida cuando termina
  la corrida (un caso borde del criterio de evacuado) y, de forma esporádica, el \emph{livelock}
  de contacto con muros del modelo de fuerzas en la jamba de un portón de la planta baja. El
  descenso por la escalera fluye sin atascos permanentes en todo el rango; el remanente es un
  límite conocido del CPM, acotado a un agente aislado.
\end{itemize}
...
\bibitem{martin2024}
R.~F.~Martin y D.~R.~Parisi,
``Anisotropic contractile particle model with avoidance for simulating
pedestrian navigation in dilute and dense systems'',
Physica A: Statistical Mechanics and its Applications \textbf{633}, 129414 (2024).
