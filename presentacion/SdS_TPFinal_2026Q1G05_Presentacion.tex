  ...
  \begin{columns}[T]
    \begin{column}{0.62\textwidth}
      \begin{itemize}
        ...
        \item Enfoque \textbf{híbrido}: posiciones y velocidades pasan a 3D, pero la
          dinámica sigue siendo \textbf{planar} en cada planta; la $z$ se interpola por
    ...
    \begin{column}{0.36\textwidth}
      ...
      \begin{tikzpicture}[scale=0.72]
        % planta baja
        \draw[thick,fill=blue!8] (0,0) -- (4,0) -- (5.4,1.1) -- (1.4,1.1) -- cycle;
        % planta alta
        \draw[thick,fill=blue!8] (0,2.6) -- (4,2.6) -- (5.4,3.7) -- (1.4,3.7) -- cycle;
        % escalera
        \draw[thick,fill=orange!45] (3.6,0.55) -- (4.3,0.55) -- (5.1,3.15) -- (4.4,3.15) -- cycle;
        % agentes
        \fill[red!70!black] (1.3,0.5) circle (0.10);
        \fill[red!70!black] (2.4,0.75) circle (0.10);
        \fill[green!55!black] (2.0,3.1) circle (0.10);
        \fill[green!55!black] (3.0,3.35) circle (0.10);
        \fill[red!70!black] (4.45,1.6) circle (0.10);
        \node[font=\scriptsize,anchor=east] at (-0.05,0.55) {$z=0$};
        \node[font=\scriptsize,anchor=east] at (-0.05,3.15) {$z=3$};
        \node[font=\scriptsize,anchor=west,orange!60!black] at (5.15,1.9) {escalera};
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[c]{Qué cambió en cada módulo}
  \small
  \begin{center}
  \begin{tabular}{@{}p{3.0cm}p{9.6cm}@{}}
    ...
    Vecinos (CIM) & Una grilla por planta + índice ``puente'' en la escalera; se preservan
      paredes y sus vértices como vecinos. \\
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{tikzpicture}[scale=0.85]
        % huella total (vista en planta)
        \draw[very thick] (0,0) rectangle (5.2,2.6);
        % descanso a la izquierda
        \draw[thick] (1.5,0) -- (1.5,2.6);
        \fill[blue!10] (0,0) rectangle (1.5,2.6);
        % separación entre tramos
        \draw[thick] (1.5,1.3) -- (5.2,1.3);
        \fill[orange!25] (1.5,0) rectangle (5.2,1.3);
        \fill[orange!45] (1.5,1.3) rectangle (5.2,2.6);
        \draw[thick] (1.5,1.3) -- (5.2,1.3);
        \draw[thick] (1.5,0) -- (1.5,2.6);
        % flechas de recorrido
        \draw[-{Latex[length=2.5mm]},very thick,blue!60!black] (4.8,0.65) -- (2.0,0.65);
        \draw[-{Latex[length=2.5mm]},very thick,blue!60!black]
          (1.1,0.65) .. controls (0.45,0.65) and (0.45,1.95) .. (1.1,1.95);
        \draw[-{Latex[length=2.5mm]},very thick,blue!60!black] (2.0,1.95) -- (4.8,1.95);
        % etiquetas
        \node[font=\scriptsize,anchor=west] at (5.25,0.65) {pie ($z=0$)};
        \node[font=\scriptsize,anchor=west] at (5.25,1.95) {tope ($z=\SI{3}{m}$)};
        \node[font=\scriptsize,align=center] at (0.75,-0.35) {descanso\\($z=\SI{1.5}{m}$)};
      \end{tikzpicture}
    \end{column}
  \end{columns}
\end{frame}
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{itemize}
        ...
        \item Escalera: ancho \SI{2.6}{m}; velocidad efectiva en el tramo
          $\approx\SI{0.59}{m/s}$.
        \item \textbf{5 realizaciones} por punto (semillas 1--5): se reporta
...
\begin{frame}[c]{El tiempo de evacuación crece con la capacidad}
  \begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
      \textbf{Observaciones}
      \begin{itemize}
        \item Al triplicar la capacidad, el promedio pasa de $48.2\pm0.6$ a
          $66.7\pm3.6$~s y el máximo de $93.5\pm3.2$ a $151.2\pm10.8$~s.
        \item La dispersión entre realizaciones crece con $N$: más congestión, más
          sensibilidad a la realización.
        \item Barras de error: $\pm\,\sigma$ entre las 5 realizaciones; en los puntos
          de menor dispersión la barra queda dentro del marcador.
        \item Queda $\approx 1.4$ agente por corrida sin evacuar (livelock de jamba de
          puerta, límite conocido del modelo de fuerzas).
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[c]{Repartir la llegada descongestiona la zona}
  \begin{columns}[T]
    ...
    \begin{column}{0.42\textwidth}
      \textbf{Observaciones}
      \begin{itemize}
        \item Concentrar la llegada en \SI{1}{min} produce un pico de $53.4\pm1.8$
          agentes; repartirla en \SI{10}{min} lo baja a $5.0\pm0.7$
          ($\sim$11$\times$ menos).
...
\begin{frame}[c]{Estudio complementario: la congestión crece con la cantidad de alumnos}
  ...
  \begin{itemize}
    ...
    \item Triplicar $N_{\max}$ multiplica el pico $\sim$11$\times$ ($4.8 \to 51.6$): la
...
\begin{frame}[c]{Conclusiones}
  \begin{itemize}
    ...
    \item La \textbf{ocupación del kiosco crece al acortar la ventana de llegada} y, a
      ...
      rango explorado.
    \item Límite conocido: $\approx 1.4$ agente por corrida no evacúa por el livelock
      de jamba de puerta del modelo de fuerzas (mitigado, no eliminado).
  \end{itemize}
\end{frame}
