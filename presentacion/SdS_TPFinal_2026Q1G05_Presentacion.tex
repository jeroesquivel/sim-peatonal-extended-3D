...
\newcommand{\videolinkEvacAlta}{}     % evacuación, capacidad alta  (N = 500)
...
\begin{frame}[c]{Estudio complementario: la congestión crece con la cantidad de alumnos}
  ...
  \vspace{2pt}
  \begin{itemize}
    \item A ventana fija, se varía la cantidad total $N_{\max}\in\{60,\dots,300\}$.
    \item Hasta saturar el kiosco ($N_{\max}\approx 180$) el pico crece \textbf{más que
      proporcionalmente} ($4.6 \to 51.4$, $\sim$11$\times$ al triplicar); después crece
      $\sim$\textbf{linealmente} ($51.4 \to 78.8 \to 104.0$): el servidor saturado atiende a
      tasa fija y el excedente se acumula en la cola.
  \end{itemize}
\end{frame}
