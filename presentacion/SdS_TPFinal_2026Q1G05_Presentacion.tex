...
\usepackage{hyperref}
\usepackage{multimedia}   % \movie: video linkeado desde videos/; pympress lo reproduce con autoplay/loop (en Adobe Reader reproduce embebido)
...
%     Se definen con catcode `_`=12 para que los guiones bajos de los IDs de
%     YouTube no rompan \href/\url (hyperref). Usar \gdef (global) por el grupo.
\begingroup\catcode`\_=12\relax
\gdef\videolinkEvacBaja{https://youtu.be/GSnIwkEzUjQ}%   evacuación, capacidad baja  (N = 40)
\gdef\videolinkEvacAlta{https://youtu.be/9H_JlijpAMA}%   evacuación, capacidad alta  (N = 500)
\gdef\videolinkIngresoUno{https://youtu.be/_g14NaB1mp8}% ingreso, ventana de 1 minuto
\gdef\videolinkIngresoDiez{https://youtu.be/9h85AAXzKDo}% ingreso, ventana de 10 minutos
\endgroup
...
%       PRESENTABLE : \embedvideotrue  + \showlinkfalse -> .mp4 vía \movie
%                     (multimedia; pympress lo reproduce), SIN URL en pantalla. ---
\newif\ifembedvideo
\newif\ifshowlink
...
\newcommand{\simvideo}[2]{%
  \begin{minipage}{\linewidth}\centering
    ...
    \ifembedvideo
      % \movie (paquete multimedia, mismo método que TP3/TP4/TP5): pympress
      % reproduce videos/#1.mp4 con autostart/loop sobre el póster. El .mp4 se
      % linkea por ruta relativa -> la carpeta videos/ debe ir JUNTO al PDF.
      \movie[autostart,loop,width=1.0115\vidH,height=\vidH,poster]{%
        \includegraphics[width=1.0115\vidH,height=\vidH]{videos/#1.png}%
      }{videos/#1.mp4}%
    \else
      \ifx\sv@link\@empty
  ...
  \begin{columns}[T]
    \begin{column}{0.62\textwidth}
      \begin{itemize}
        ...
        \item Enfoque \textbf{híbrido}: las posiciones pasan a 3D, pero la dinámica sigue
          \textbf{planar} en cada planta; la $z$ se interpola por el avance sobre la
          escalera (no hay $v_z$).
      \end{itemize}
    \end{column}
    \begin{column}{0.36\textwidth}
      \centering
      \resizebox{\linewidth}{!}{\input{diagrams/diag_plantas.tex}}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[plain,c]
  \centering
  {\usebeamercolor[fg]{structure}\fontsize{40}{48}\selectfont Implementación\par}
\end{frame}
...
\begin{frame}[c]{Qué cambió en cada módulo, y por qué}
  ...
  \begin{tabular}{@{}p{2.4cm}p{6.2cm}p{4.0cm}@{}}
    ...
    Vecinos (CIM) & Una grilla por planta + índice ``puente'' en la escalera; paredes y
      vértices como vecinos. & La vecindad es local a cada planta. \\
...
\begin{frame}[c]{Escaleras \emph{switchback} con descanso}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \centering
      \resizebox{\linewidth}{!}{\input{diagrams/diag_switchback.tex}}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[c]{Lo que sólo aparece con dos plantas reales}
  \begin{itemize}
    \item \textbf{Verde en aislamiento, roto en integración}: la $z$ se perdía \emph{en
      silencio} y nadie subía a la planta alta, sin un solo \emph{crash}.
    \item \textbf{El ruteo se quedaba al pie} de la escalera $\Rightarrow$ se cruza
      explícito la \textbf{arista de escalera}.
    \item \textbf{Salto de $z$ al pie}: la escalera queda fuera de la grilla y el cambio de
      \emph{hop} va pegado al extremo.
    \item \textbf{Livelock en jambas}: el contacto pasa a ser sólo al núcleo duro
      ($r_{\min}$); la repulsión suave dobla esquinas sin oscilar.
    \item \textbf{Lección}: al sumar una coordenada, auditar \emph{todos} los caminos de
      datos; los \emph{defaults} silenciosos engañan.
  \end{itemize}
\end{frame}
...
\begin{frame}[c]{Qué medimos en la evacuación}
  \begin{itemize}
    \item Los $N$ alumnos comienzan \textbf{dentro de las aulas} de ambas plantas y
      ...
      por las escaleras.
    \item \textbf{Input}: la capacidad total $N \in \{40, 80, 120, 200, 300, 400, 500\}$.
...
\begin{frame}[c]{La evacuación desde ambas plantas}
  \begin{columns}[T]
    ...
    \begin{column}{0.5\textwidth}
      \centering
      \simvideo{anim_evac_n500}{\videolinkEvacAlta}\\[1pt]
      {\footnotesize Capacidad alta: $N=500$.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[c]{La distribución de tiempos de evacuación es bimodal}
  ...
  {\footnotesize Densidad con banda $\pm\sigma$ (subconjunto de $N$).}
  \begin{itemize}
    ...
    \item Al crecer $N$, el lóbulo lento se desplaza a tiempos mayores y se ensancha,
      hasta dominar la distribución en las capacidades grandes.
  \end{itemize}
\end{frame}
    ...
    \begin{column}{0.42\textwidth}
      ...
      \begin{itemize}
        \item El edificio se vacía completo en todo el rango: de $N=40$ a $500$
          el promedio pasa de $38.9$ a $108.7$~s ($\times2.8$).
        \item El \textbf{máximo} crece más rápido, de $57.0$ a $305.2$~s
          ($\times5.4$): lo gobierna el agente más rezagado del lóbulo lento.
...
\begin{frame}[c]{La huella espacial de la evacuación}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      ...
      {\footnotesize Densidad de ocupación.}
    \end{column}
    \begin{column}{0.5\textwidth}
      \centering
      \includegraphics[width=\linewidth]{heatmap_tevac.png}\\[2pt]
      {\footnotesize $t_{\text{evac}}$ medio por origen.}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}[c]{Estudio complementario: la congestión crece con la cantidad de alumnos}
  ...
  \vspace{2pt}
  \begin{itemize}
    \item A ventana fija ($N_{\max}\in\{60,\dots,300\}$): el pico crece \textbf{más que
      proporcionalmente} hasta saturar el kiosco ($N_{\max}\approx 180$) y luego
      $\sim$\textbf{linealmente}.
  \end{itemize}
\end{frame}
...
\begin{frame}[c]{La huella espacial del ingreso}
  ...
  {\footnotesize Densidad de ocupación durante el ingreso.}
\end{frame}
